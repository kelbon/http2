...
#include <yacore/utils/string_switch.hpp>
...
dd::task<http_response> http2_client::sendRequest(http_request request,
                                                  deadline_t deadline) {
  http_response rsp;
  ...
  rsp.status = co_await sendRequest(&onHeader, &onDataPart, std::move(request),
                                    deadline);
  if (rsp.status < 0) {
...
void enum_from_string(std::string_view str, scheme_e &s) noexcept {
  using enum scheme_e;
  s = utils::string_switch<scheme_e>(str)
          .case_("http", HTTP)
          .case_("https", HTTPS)
          .orDefault(UNKNOWN);
}
