...
#include "http2v2/http2_client.hpp"
...
#include "http2v2/http2_writer.hpp"
#include "http2v2/logger.hpp"
#include "http2v2/utils/macro.hpp"
#include "http2v2/utils/reusable_buffer.hpp"
...
void http2_client::notifyConnectionWaiters(
    http2_connection_ptr_t result) noexcept {
  // assume only i have access to waiters
  auto waiters = std::move(m_connectionWaiters);
  HTTP2_LOG(TRACE, "resuming connection waiters, count: {}, connection: {}",
            waiters.size(), (void *)result.get());
...
struct ping_callback {
  ...
  http2_client_ptr client = nullptr;

  // precondition: c != nullptr, client != nullptr
  explicit ping_callback(http2_connection_ptr_t c, http2_client_ptr clientptr,
                         duration_t pingTimeout) {
    assert(c);
    assert(clientptr);
    con = std::move(c);
  ...
  void operator()() {
    ...
    seastar::engine().run_in_background(
        [](http2_connection_ptr_t c) -> seastar::future<> {
          (void)co_await send_ping(c, PING_VALUE, /*requestPong=*/true);
        }(con));
  }
};

dd::job http2_client::startConnecting(http2_client_ptr self,
                                      deadline_t deadline) {
  assert(self);
  ...
  ON_SCOPE_EXIT { self->m_connectionGate.leave(); };
  ...
  try {
    ...
    // * no new requests, all stopped.
    {
      ...
      HTTP2_LOG(TRACE, "creating connection for client {}", (void *)self.get());
      ON_SCOPE_EXIT {
        lock.release();
        self->notifyConnectionWaiters(newConnection);
        HTTP2_LOG(TRACE, "new connection: {}", (void *)newConnection.get());
      };
      any_connection_t tcpCon = co_await self->m_factory->createConnection(
          self->getHost(), self->getSrcAddr(), deadline);
      http2_connection_ptr_t con = new http2_connection(std::move(tcpCon));
      ...
      ON_SCOPE_EXIT { self->m_notYetReadyConnection = nullptr; };
      newConnection = co_await establish_http2_session_client(std::move(con),
                                                              self->m_options);
    ...
    auto sleepcb = [self](duration_t d, io_error_code &ec) {
      return self->m_factory->sleep(d, ec);
    };
    auto onnetworkerr = [self] { self->dropConnection(reqerr_e::NETWORK_ERR); };
    start_writer_for_client(newConnection, std::move(sleepcb),
                            std::move(onnetworkerr),
                            self->getOptions().forceDisableHpack,
                            self->m_connectionPartsGate.hold());

    if (self->m_options.pingInterval != duration_t::max()) {
      newConnection->pingtimer.arm_periodic(self->m_options.pingInterval);
      newConnection->pingtimer.set_callback(
          ping_callback(newConnection, self, self->m_options.pingTimeout));
      // armed when ping sended, canceled when ping received
      newConnection->pingdeadlinetimer.set_callback(
          [self] { self->dropConnection(reqerr_e::TIMEOUT); });
    }
    ...
    newConnection->timeoutWardenTimer.set_callback([newConnection] {
      ...
      if (!newConnection->timers.empty()) {
        newConnection->timeoutWardenTimer.arm(
            newConnection->timers.top()->deadline.tp);
      }
    });
    // Note: after setting m_connection, so connected() returns true
    if (self->m_onconnect) {
      HTTP2_LOG(DEBUG, "onConnect called");
      self->m_onconnect();
    }
...
static bool handle_utility_frame(http2_frame_t frame, http2_connection &con) {
  ...
  case SETTINGS:
    ...
    return true;
  case PING:
    handle_ping(ping_frame::parse(frame.header, frame.data), &con)
        .start_and_detach();
    return true;
  case RST_STREAM:
    if (!con.finishStreamWithError(
            rst_stream::parse(frame.header, frame.data))) {
      HTTP2_LOG(INFO,
  ...
  case GOAWAY: {
    ...
    if (f.errorCode != errc_e::NO_ERROR) {
      throw goaway_exception(f.lastStreamId, f.errorCode,
                             std::move(f.debugInfo));
    } else {
...
handle_headers_or_data(http2_frame_t frame, http2_connection &con) noexcept {
  using enum frame_e;
  ...
  try {
    ...
    HTTP2_LOG(ERROR, "exception while decoding headers block (HPACK), err: {}",
              e.what());
    ...
    // identifier
    send_goaway(&con, MAX_STREAM_ID, errc_e::COMPRESSION_ERROR, e.what())
        .start_and_detach();
    return false;
  } catch (protocol_error &e) {
    HTTP2_LOG(ERROR, "exception while handling frame for stream {}, err: {}",
...
[[nodiscard]] static bool handle_frame(http2_frame_t frame,
...
dd::job http2_client::startReaderFor(http2_client_ptr self,
                                     http2_connection_ptr_t c) {
  using enum frame_e;
  HTTP2_LOG(TRACE, "reader started for {}", (void *)c.get());
  assert(self && c);
  assert(!self->m_connectionPartsGate.is_closed());

  ON_SCOPE_EXIT { HTTP2_LOG(TRACE, "reader for {} ended", (void *)c.get()); };
  ...
  try {
    ...
    HTTP2_LOG(ERROR, "goaway received, info: {}, errc: {}", gae.debugInfo,
              e2str(gae.errorCode));
...
protocol_error:
  ...
  co_await self->m_factory->sleep(std::chrono::milliseconds(3), ec);
  reason = reqerr_e::PROTOCOL_ERR;
  goto dropConnection;
network_error:
  reason = ec == io_error_code_e::operation_aborted ? reqerr_e::CANCELLED
                                                    : reqerr_e::NETWORK_ERR;
  if (reason == reqerr_e::NETWORK_ERR) {
    HTTP2_LOG(DEBUG, "reader drops connection after network err: {}",
              ec.what());
  }
dropConnection:
...
connection_dropped:
  if (!self->m_connectionWaiters.empty() && !self->alreadyConnecting()) {
    HTTP2_LOG(
        DEBUG,
        "client initiates reconnect after graceful shutdown or out of streams");
    co_await dd::this_coro::destroy_and_transfer_control_to(
        startConnecting(self, deadline_after(self->m_options.connectionTimeout))
            .handle);
  }
  co_return;
}
...
http2_client::http2_client(sockaddr_or_fqdn_t host, http2_client_options opts,
                           any_transport_factory tf)
    : m_host(std::move(host)), m_options(opts), m_factory(std::move(tf)) {
  assert(m_factory);
  m_options.maxReceiveFrameSize =
      std::min(FRAME_LEN_MAX, m_options.maxReceiveFrameSize);
...
std::coroutine_handle<> noexport::waiter_of_connection::await_suspend(
  ...
  return client->startConnecting(client->shared_from_this(), deadline).handle;
}

[[nodiscard]] http2_connection_ptr_t
noexport::waiter_of_connection::await_resume() {
...
dd::task<int> http2_client::sendRequest(on_header_fn_ptr onHeader,
                                        on_data_part_fn_ptr onDataPart,
                                        http_request request,
                                        deadline_t deadline) {
  http2_client_ptr self = shared_from_this();
  ...
  ON_SCOPE_EXIT { --m_requestsInProgress; };
  ...
  assert(!request.authority.empty());
  ...
  HTTP2_LOG(TRACE, "sending http2 request, path: {}, method: {}, streamid: {}",
            request.path, (int)request.method, streamid);

  node_ptr node = con->newRequestNode(std::move(request), deadline, onHeader,
                                      onDataPart, streamid);

  co_return co_await con->responseReceived(*node);
}

seastar::future<void> http2_client::coStop() {
  http2_client_ptr self = shared_from_this();
  m_onconnect = nullptr;
  ...
  http2_connection_ptr_t con =
      m_connection; // prevent destroy for graceful shutdown.
  if (con && con->tcpCon) {
    con->tcpCon->deinit();
  }
  ...
  {
    co_await m_factory->sleep(std::chrono::nanoseconds(10), ec);
  }
  // request stop
  ++m_stopRequested;
  ON_SCOPE_EXIT { --m_stopRequested; };
...
seastar::future<void> http2_client::coAbort() {
  dropConnection(reqerr_e::CANCELLED);
  notifyConnectionWaiters(nullptr);
  return coStop();
}

bool http2_client::connected() const { return !!m_connection; }
...
void http2_client::setHost(sockaddr_or_fqdn_t s) noexcept {
  assert(!connected());
  m_host = std::move(s);
}

void http2_client::setOnConnect(std::function<void()> foo) {
  m_onconnect = std::move(foo);
}

seastar::future<bool> http2_client::connect() {
  http2_client_ptr self = shared_from_this(); // keep *this alive
  if (!m_connectionGate.try_enter()) {
    co_return false;
  }
  ON_SCOPE_EXIT { m_connectionGate.leave(); };
  io_error_code ec;
  while (!stopRequested()) {
    http2_connection_ptr_t con =
        co_await borrowConnection(deadline_after(m_options.connectionTimeout));
    if (m_connectionGate.is_closed()) {
      co_return false;
    }
    if (con) {
      co_return true;
    ...
    co_await m_factory->sleep(std::chrono::seconds(1), ec);
    if (ec) {
      break;
    }
  }
...
seastar::future<bool> http2_client::tryConnect(deadline_t deadline) {
  http2_client_ptr self = shared_from_this();
