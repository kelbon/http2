...
dd::task<void> send_echo_request(fuzzer& fuz, h2client& c, hreq req);

// sends requests, but body will be splitted into random chunks
// also validates result
dd::task<void> send_echo_request_as_stream(fuzzer& fuz, h2client& c, hreq req);

// sends request, but body will be splitted into random chunks + expects server answers stream
// also validates result
dd::task<void> send_echo_request_connect(fuzzer& fuz, h2client& c, hreq req, bool websocket);
...
dd::task<void> emulate_client_n(fuzzer&, h2client&, any_reqtem tem, size_t request_count,
...
dd::task<void> emulate_client(fuzzer&, h2client&, any_reqtem tem, duration_t dur,
                              size_t max_active_streams = 10, req_weights = {});
