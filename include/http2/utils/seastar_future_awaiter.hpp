...
template <typename T>
auto wait_future_noexcept(seastar::future<T> f) {
  return seastar_future_awaiter_noexcept<T>(std::move(f));
}

template <typename T>
auto wait_future(seastar::future<T> foo) {
  return seastar_future_awaiter<T>(std::move(foo));
}

template <typename T>
auto wait_future(seastar::future<T> foo, deadline_t deadline) {
  return seastar_future_awaiter<T>(seastar::with_timeout(deadline.tp, std::move(foo)));
}
