...
constexpr inline cfint_t INITIAL_WINDOW_SIZE_FOR_CONNECTION_OVERALL =
    uint32_t(65'535);
constexpr inline cfint_t MAX_WINDOW_SIZE =
    (uint32_t(1) << 31) - 1; // 2'147'483'647
...
constexpr inline flags_t ACK =
    0x01; // SETTINGS, PING, means ping answer or settings accept
...
struct frame_header {
  ...
  template <std::output_iterator<hpack::byte_t> O> O form(O out) const {
  ...
  [[nodiscard]] static frame_header
  parse(std::span<hpack::byte_t const> rawheader) {
    assert(rawheader.size() == FRAME_HEADER_LEN);
    frame_header h;
    h.length = uint32_t(rawheader[0] << 16) | uint32_t(rawheader[1] << 8) |
...
struct rst_stream {
  ...
  [[nodiscard]] static rst_stream parse(frame_header h,
...
struct settings_t {
  static constexpr inline uint32_t MAX_MAX_CONCURRENT_STREAMS =
      ((uint32_t(1) << 31) - 1);
...
struct [[gnu::packed]] setting_t {
  ...
  template <std::output_iterator<byte_t> O> static O form(setting_t s, O out) {
    s.identifier = htonl_value(s.identifier);
    s.value = htonl_value(s.value);
    return std::copy_n(as_bytes(s).data(), sizeof(s), out);
  }

  [[nodiscard]] static setting_t
  parse(std::span<byte_t const, 6> bytes) noexcept {
    setting_t s;
...
struct ping_frame {
  ...
  [[nodiscard]] static ping_frame parse(frame_header h,
                                        std::span<byte_t const> bytes);
};
...
struct goaway_frame {
  ...
  */
  ...
  [[noreturn]] static void parseAndThrowGoaway(frame_header header,
                                               std::span<byte_t const> bytes) {
  ...
  template <std::output_iterator<byte_t> O>
  static O form(stream_id_t lastStreamId, errc_e errorCode,
                std::string debugInfo, O out) {
    out =
        frame_header{
...
struct window_update_frame {
  ...
  */
  ...
  [[nodiscard]] static window_update_frame parse(frame_header header,
                                                 std::span<byte_t const> bytes);
};

// TODO YAC-29725 YAC-36559
...
inline void
increment_window_size_trusted(cfint_t &size,
                              int32_t windowSizeIncrement) noexcept {
  assert(windowSizeIncrement > 0);
  assert(int64_t(size) + int64_t(windowSizeIncrement) <=
...
inline void decrease_window_size(cfint_t &size, int32_t decrease) {
  ...
  size -= decrease;
  if (size < 0) [[unlikely]] {
    HTTP2_LOG(WARN, "window size is < 0 ( {} ) after decreasing by {}", size,
              decrease);
  }
