...
struct reqerr_e {
  ...
  enum server_request_progress_e : int {
    REQUEST_CREATED =
        -10, // стрим без владельца (detach), сохранён в con.responses
    RESPONSE_IN_PROGRESS =
        -11, // корутина send_response создана и владеет стримом
  };
};
...
struct http_header_t {
  ...
  std::string_view name() const noexcept { return hname; }
  std::string_view value() const noexcept { return hvalue; }
};
...
template <>
struct formatter<::http2v2::http_header_t> : formatter<std::string_view> {
  auto format(::http2v2::http_header_t const &hdr, auto &ctx) const {
    return fmt::format_to(ctx.out(), "{}: {}", hdr.name(), hdr.value());
  }
};
