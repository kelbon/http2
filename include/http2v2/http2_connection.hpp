  ...
  void ignoreDeprecatedPriority() {
    ...
    if (data.size() < 5) {
      HTTP2_LOG(ERROR,
                ...
                header.length, header.streamId, header.flags, (int)header.type,
                data.size());
...
struct request_node {
  ...
  [[nodiscard]] bool finished() const noexcept { return task == nullptr; }
  ...
  struct compare_by_deadline {
    bool operator()(request_node const &l,
                    request_node const &r) const noexcept {
...
struct http2_connection {
  using requests_member_hook_t =
      bi::member_hook<request_node, bi::list_member_hook<link_option_t>,
                      &request_node::requestsHook>;
  using responses_member_hook_t =
      bi::member_hook<request_node,
                      bi::unordered_set_member_hook<link_option_t>,
                      &request_node::responsesHook>;
  using requests_t =
      bi::list<request_node, bi::cache_last<true>, requests_member_hook_t>;
  using responses_t = bi::unordered_set<
      request_node, bi::constant_time_size<true>, responses_member_hook_t,
  ...
  using timers_member_hook_t =
      bi::member_hook<request_node, bi::bs_set_member_hook<link_option_t>,
                      &request_node::timersHook>;
  using timers_t =
      bi::treap_multiset<request_node, bi::constant_time_size<true>,
                         timers_member_hook_t,
  ...
  seastar::timer<std::chrono::steady_clock> pingtimer;
  seastar::timer<std::chrono::steady_clock> pingdeadlinetimer;
  seastar::timer<std::chrono::steady_clock> timeoutWardenTimer;
  bi::slist<request_node, requests_member_hook_t, bi::constant_time_size<true>>
      freeNodes;

  explicit http2_connection(any_connection_t &&c);
  ...
  [[nodiscard]] bool isDropped() const noexcept { return dropped; }
  void startDrop() noexcept { dropped = true; }
  ...
  struct work_waiter {
    ...
    void await_suspend(
        std::coroutine_handle<dd::job_promise> writer) noexcept // NOLINT
    {
      assert(connection->writer.handle == nullptr);
      connection->writer.handle = writer;
    }
    [[nodiscard]] bool await_resume() const noexcept // NOLINT
    {
  ...
  [[nodiscard]] work_waiter waitWork() noexcept { return work_waiter(this); }
  ...
  void finishRequestWithUserException(request_node &node,
  ...
  node_ptr newRequestNode(http_request &&request, deadline_t deadline,
                          on_header_fn_ptr onHeader,
  ...
  struct response_awaiter {
    ...
    [[nodiscard]] int await_resume() const noexcept // NOLINT
    {
      return n->status;
    }
  };
  ...
  KELCORO_CO_AWAIT_REQUIRED response_awaiter
  responseReceived(request_node &node) noexcept;
};
