...
#include "http2v2/http2_client_options.hpp"
...
struct waiter_of_connection : bi::list_base_hook<link_option_t> {
  ...
  explicit waiter_of_connection(http2_client *c, deadline_t dl) noexcept
      : client(c), deadline(dl) {}

  ~waiter_of_connection();

  bool await_ready() noexcept; // NOLINT
  std::coroutine_handle<>
      await_suspend(std::coroutine_handle<>) noexcept; // NOLINT
  [[nodiscard]] http2_connection_ptr_t await_resume(); // NOLINT
};
...
using http2_client_ptr = seastar::shared_ptr<http2_client>; // NOLINT

// Note: not RAII type, .stop() must be awaited before destruction. Wrapped then
// in RAII http2::Client
struct http2_client : seastar::enable_shared_from_this<http2_client> {
protected:
  friend noexport::waiter_of_connection;

  sockaddr_or_fqdn_t m_host;
  seastar::socket_address m_srcAddr;
  ...
  bi::list<noexport::waiter_of_connection, bi::cache_last<true>>
      m_connectionWaiters;
  size_t m_requestsInProgress = 0;
  size_t m_isConnecting =
      0; // if connection started to establish, but not established yet
  ...
  seastar::gate m_connectionGate;
  // for connection reader/writer
  seastar::gate m_connectionPartsGate;

  std::function<void()> m_onconnect; // TODE rm

  // fills requests from raw http2 frames
  static dd::job startReaderFor(http2_client_ptr, http2_connection_ptr_t);

  // postconditon: returns not null, !returned->dropped && returned->stream_id
  // <= MAX_STREAM_ID
  // && !client.stop_requestedg
  [[nodiscard]] noexport::waiter_of_connection
  borrowConnection(deadline_t deadline) noexcept {
  ...
  [[nodiscard("this handle must be resumed")]] static dd::job
      startConnecting(http2_client_ptr, deadline_t);

  bool stopRequested() const noexcept { return m_stopRequested > 0; }
...
public:
  ...
  explicit http2_client(sockaddr_or_fqdn_t host, http2_client_options,
                        any_transport_factory);

  http2_client(http2_client &&) = delete;
  void operator=(http2_client &&) = delete;

  sockaddr_or_fqdn_t const &getHost() const noexcept { return m_host; }
  seastar::socket_address const &getSrcAddr() const noexcept {
    return m_srcAddr;
  }
  void setSrcAddr(seastar::socket_address addr) { m_srcAddr = addr; }

  void setConnectionTimeout(duration_t dur) noexcept {
    m_options.connectionTimeout = dur;
  }
  http2_client_options const &getOptions() const noexcept { return m_options; }
  ...
  // response code
  dd::task<int> sendRequest(on_header_fn_ptr onHeader,
                            on_data_part_fn_ptr onDataPart, http_request,
                            deadline_t deadline);

  // throws on errors
  dd::task<http_response> sendRequest(http_request, deadline_t);

  dd::task<http_response> sendRequest(http_request request,
                                      duration_t timeout) {
  ...
  void setHost(sockaddr_or_fqdn_t) noexcept;

  // returns true if client connected after future resoloves
  seastar::future<bool> tryConnect(deadline_t);

  // returns true if client connected after future resoloves
  seastar::future<bool> tryConnect(duration_t d) {
    return tryConnect(deadline_after(d));
  }

  // returns true if client connected after future resoloves
  seastar::future<bool> tryConnect() {
    return tryConnect(deadline_after(m_options.connectionTimeout));
  }

  // бесконечно пытается соединиться до успеха или до вызова .stop
  seastar::future<bool> connect();
  ...
  // == false
  seastar::future<void> coStop();

  // aborts all requsts and stops
  seastar::future<void> coAbort();
