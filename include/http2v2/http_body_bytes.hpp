...
template <typename T> struct allocator_p9 {
  static_assert(std::is_same_v<T, byte_t> || std::is_same_v<noinit, T>);
  using value_type = T;
  using size_type = size_t;
  using difference_type = ptrdiff_t;
  using propagate_on_container_move_assignment = std::true_type; // NOLINT
...
constexpr inline uninitialized_byte_t uninitialized_byte =
    uninitialized_byte_t{}; // NOLINT

struct http_body_bytes
    : private std::vector<byte_t, detail::allocator_p9<byte_t>> {
...
private:
  ...
  [[gnu::noinline]] void resizeNoinit(void *p, size_t sz) {
    ...
    // (memset)
    using dirty_hack_t =
        std::vector<detail::noinit, detail::allocator_p9<detail::noinit>>;
